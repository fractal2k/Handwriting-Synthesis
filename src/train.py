...
parser.add_argument(
    '-l', '--log', help='Name of the log file for this training run.')
parser.add_argument('-e', '--epochs', default=1, type=int,
                    help='Number of training epochs')
parser.add_argument('-c', '--checkpoint', default=False, nargs='?',
                    type=str2bool, help='Initialize models from last checkpoint')
args = parser.parse_args()

# Define global constants
device = torch.device('cuda:1' if torch.cuda.is_available() else 'cpu')
current_log = args.log
writer = SummaryWriter(f"/home/sujayrokade/hsynthesis/src/runs/{current_log}")
...
def init_optim(lm, gen, dis, rec):
    ...
    lm_opt = optim.Adam(lm.parameters(),
    ...
    dis_opt = optim.Adam(dis.parameters(),
                         lr=config.LEARNING_RATE,
                         betas=config.BETAS)
    rec_opt = optim.Adam(rec.parameters(),
                         lr=config.LEARNING_RATE,
                         betas=config.BETAS)
...
def train(epochs=1, from_checkpoint=False, checkpoint_interval=1):
    ...
    # Clip gradients of rec
    for _, param in rec.named_parameters():
        param.register_hook(lambda grad: clip_norm(grad, 2))
    ...
    for epoch in range(epochs):
        ...
        for batch in tqdm(trainloader):
            ...
            # imgs = (imgs + 0.25 * torch.randn_like(imgs)).to(device)
            imgs = imgs.to(device)
            ...
            # Adding noise to discriminator input
            # gen_out_dis = gen_out_dis + 0.25 * torch.randn_like(gen_out_dis)
            dis_out_fake = dis(gen_out_dis)
            dis_out_real = dis(imgs)
            rec_out_dis = rec(imgs)
            dis_loss = dis_criterion(dis_out_fake, dis_out_real)
            rec_loss = compute_ctc_loss(
                ctc_criterion, rec_out_dis, ctc_labels, lens)
            ...
            try:
                for name, param in dis.named_parameters():
                    writer.add_scalar(
                        f"dis gradnorm {name}", param.grad.norm(), count)
            except:
            ...
            try:
                for name, param in rec.named_parameters():
                    writer.add_scalar(
                        f"rec gradnorm {name}", param.grad.norm(), count)
            except:
            ...
            gen_out = gen(z, emb)
            # Adding noise to discriminator input
            # gen_out = gen_out + 0.25 * torch.randn_like(gen_out)
            dis_out = dis(gen_out)
            rec_out = rec(gen_out)
            ...
            try:
                for name, param in gen.named_parameters():
                    writer.add_scalar(
                        f"gen gradnorm {name}", param.grad.norm(), count)
            except:
            ...
            try:
                for name, param in lm.named_parameters():
                    writer.add_scalar(
                        f"lm gradnorm {name}", param.grad.norm(), count)
            except:
